//___ Css ___//
import "./ToDo_App.css";

//___ Images ___//
import Tick from "../../assets/Images/Icons/tick.png";
import Not_Tick from "../../assets/Images/Icons/not_tick.png";
import Delete from "../../assets/Images/Icons/delete.png";

const ToDo_Items = (props) => {
  const { text } = props;

  return (
    <div className="ToDo_Items d-flex gap-10">
      <div className="toDoOnlyOneItems d-flex gap-10">
        <img src={Tick} alt="" className="c_pointer" />
        <p className="c_pointer">{text}</p>
      </div>
      <img src={Delete} alt="" className="deleteIcon c_pointer" />
    </div>
  );
};

export default ToDo_Items;
